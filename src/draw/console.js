...
export class Console {
  constructor(doc, analysis, stats, opts) {
    this.$ = id => doc.getElementById(id); this.a = analysis; this.stats = stats; this.opts = opts;
    this.el = this.$('console'); this.last = 0;
    const valid = analysis.S.filter(v => v > -39);
    this.hist = new Array(24).fill(0); for (const v of valid) this.hist[clamp(Math.floor((v + 30) / 24 * 24), 0, 23)]++;
    this.doc = doc;
  }
  ...
  render(state) {
    const { t, sky, wx, playing, live, disp, spec, events, W, H, Q, fpsCap } = state, a = this.a, st = a.at(a.S, t), mo = a.at(a.M, t), pk = a.at(a.PK, t), $ = this.$;
    $('stateLine').textContent = `${playing ? 'Lecture' : 'Pause'} à ${tc(t)} sur ${tc(a.duration)}. ${live ? 'Analyse du son en direct.' : 'Analyse précalculée, le son n’est pas accessible à la page : sers-la en HTTP ou charge le fichier avec o.'}`;
    ...
    { const [x, w, h] = this.chart('chSpec'), n = spec.length, bw = w / n; for (let i = 0; i < n; i++) { x.fillStyle = i < n / 6 ? C.sub : i < n * .42 ? C.bass : i < n * .75 ? C.mid : C.high; x.globalAlpha = .85; const bh = Math.max(2, spec[i] * (h - 18)); x.beginPath(); x.roundRect(i * bw + 1, h - 16 - bh, bw - 2, bh, [3, 3, 0, 0]); x.fill(); } x.globalAlpha = 1; x.fillStyle = C.ink2; ['40 Hz', '200 Hz', '1 kHz', '5 kHz', '12 kHz'].forEach((l, i) => { x.textAlign = i === 0 ? 'left' : i === 4 ? 'right' : 'center'; x.fillText(l, i / 4 * w, h - 3); }); }
    { const [x, w, h] = this.chart('chBands'), hist = this.stats.bandHist, n = hist.length; x.strokeStyle = C.line; x.beginPath(); for (let k = 0; k <= 2; k++) { x.moveTo(0, 6 + k * (h - 24) / 2); x.lineTo(w, 6 + k * (h - 24) / 2); } x.stroke();
    ...
    { const [x, w, h] = this.chart('chSet'), top = 4, plotH = h - 44, DUR = a.duration, xs = tt => tt / DUR * w, ys = v => top + (1 - LUF(v)) * plotH, N = Math.floor(w);
      x.strokeStyle = C.line; x.beginPath(); for (let v = -30; v <= -6; v += 8) { x.moveTo(0, ys(v)); x.lineTo(w, ys(v)); } x.stroke();
      x.beginPath(); x.moveTo(0, ys(-30)); for (let i = 0; i <= N; i++) x.lineTo(i, ys(a.at(a.S, i / N * DUR))); x.lineTo(w, ys(-30)); x.closePath(); x.fillStyle = C.series; x.globalAlpha = .18; x.fill(); x.globalAlpha = 1;
      x.strokeStyle = C.series; x.lineWidth = 1.5; x.beginPath(); for (let i = 0; i <= N; i++) { const py = ys(a.at(a.S, i / N * DUR)); i ? x.lineTo(i, py) : x.moveTo(i, py); } x.stroke();
      x.strokeStyle = C.ink2; x.setLineDash([3, 3]); x.beginPath(); x.moveTo(0, ys(a.meta.I)); x.lineTo(w, ys(a.meta.I)); x.stroke(); x.setLineDash([]); x.fillStyle = C.ink2; x.textAlign = 'left'; x.fillText(`intégré ${fr(a.meta.I)} LUFS`, 4, ys(a.meta.I) - 4);
    ...
    { const [x, w, h] = this.chart('chHist'), mx = Math.max(...this.hist), bw = w / this.hist.length, cur = clamp(Math.floor((st + 30) / 24 * 24), 0, 23);
      this.hist.forEach((v, i) => { x.fillStyle = i === cur ? C.ink : C.series; x.globalAlpha = i === cur ? 1 : .8; const bh = v / mx * (h - 20); x.beginPath(); x.roundRect(i * bw + 1, h - 16 - bh, bw - 2, bh, [3, 3, 0, 0]); x.fill(); });
      x.globalAlpha = 1; x.fillStyle = C.ink2; x.textAlign = 'left'; x.fillText('−30 LUFS', 0, h - 3); x.textAlign = 'right'; x.fillText('−6 LUFS', w, h - 3); x.textAlign = 'center'; x.fillText('la barre claire est la loudness actuelle', w / 2, h - 3); }
    { const [x, w, h] = this.chart('chFps'), fh = this.stats.fpsHist, n = fh.length, yc = 6 + (1 - fpsCap / 40) * (h - 24); x.strokeStyle = C.line; x.beginPath(); x.moveTo(0, yc); x.lineTo(w, yc); x.stroke();
    ...
    { const [x, w, h] = this.chart('chEvents'), keys = Object.keys(TYPES), rowH = h / keys.length, sp = this.stats.spawned, mx = Math.max(1, ...keys.map(k => sp[k] || 0)), on = {}; for (const e of events) on[e.type] = (on[e.type] || 0) + 1;
    ...
    if (!$('segments').children.length) {
      ...
      for (let s0 = 0; s0 < a.duration; s0 += 600) { const idx = []; for (let i = 0; i < a.S.length; i++) if (i * a.step >= s0 && i * a.step < s0 + 600 && a.S[i] > -39) idx.push(i); if (!idx.length) continue; const mean = idx.reduce((q, i) => q + a.S[i], 0) / idx.length, mxm = Math.max(...idx.map(i => a.M[i])), pkm = Math.max(...idx.map(i => a.PK[i])); const ws = [...new Set([0, 1, 2, 3].map(j => WNAME[pickW(Math.floor((s0 + j * 150) / WSEG))]))].join(', '); html += `<tr><td>${mmss(s0)} à ${mmss(Math.min(a.duration, s0 + 600))}</td><td>${fr(mean)} LUFS</td><td>${fr(mxm)} LUFS</td><td>${fr(pkm)} dBFS</td><td>${ws}</td></tr>`; }
