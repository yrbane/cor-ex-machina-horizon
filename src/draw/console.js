...
export class Console {
  ...
  render(state) {
    ...
    { const [x, w, h] = this.chart('chSet'), top = 4, plotH = h - 44, DUR = Math.max(1, isLive ? dur : a.duration), lo = isLive ? Math.min(...a.S, -30) : -30, span = isLive ? Math.max(24, Math.max(...a.S, -6) - lo) : 24, xs = tt => tt / DUR * w, ys = v => top + (1 - clamp((v - lo) / span, 0, 1)) * plotH, N = Math.floor(w);
    ...
    { const hist = this.histFor(a), [x, w, h] = this.chart('chHist'), mx = Math.max(1, ...hist), bw = w / hist.length, cur = clamp(Math.floor((st - this.histLo) / this.histSpan * 24), 0, 23);
    ...
    if (this.tableA !== a || (isLive && a.S.length - this.tableN > 40)) {
